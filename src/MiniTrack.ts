...
export default class MiniTrack {
  ...
  private trackAppStart (event:string):void{
    this.assemblyData({event,callBack:()=>{
      this.timeToUpload()
    }})
  }
  ...
  private trackError (event,error:string):void{
    ...
    this.assemblyData({event,arg:{event_name:errInfo[0],event_params:errInfo[0] + errInfo[1]}})
  }
  ...
  private trackClick (params):void{
    this.assemblyData({event:'CLICK',arg:{event_name:params.event_name,element:params.element}})
  }
  private assemblyData <T> (params:{event:string,arg?:{event_name:string,event_params?:string,element?:T},callBack?:Function}){
    const tcapData = () => {
      this.step++
      let data:IUploadData = {
        ...
        ...(params.arg && params.arg.element && {element:params.arg.element}),
        ...(params.arg && params.arg.event_params && {event_params:params.arg.event_params}),
        ...(this.device && {device:this.device}),
      }
      this.currentData.push(data)
      params.callBack && params.callBack()
    }
    this.identity ? tcapData() : this.setIdentity(()=>{tcapData()})
  }
